...
\begin{defn}
  ...
  defined as:
  \begin{align*}
    \dd{\mathtt{[\ ]}}  \quad&:\equiv\quad \mathsf{id}; \\
...
\begin{proof}
  ...
  to show that $\dd{is'} \circ k_i$ is a prenucleus. For meet preservation,
...
\end{prop}
\begin{proof}
  $K^*$ is always inhabited by $\dd{\mathtt{[\ ]}}$. Upwards-closure also holds since,
  given $is, js : \mathsf{List}(I)$, $\dd{is \append js}$ is the upper bound of $\{
  \dd{is}, \dd{js} \}$: $\dd{is \append js} = \dd{is} \circ
  \dd{js}$ (by Proposition~\ref{prop:app-lemma}) which is the upper bound of $\{
  \dd{is}, \dd{js} \}$ by Proposition~\ref{prop:nucl-lemma-1}.
\end{proof}
