...
We will also need the following important property of nuclei.
...
We now refine the meet-semilattice of nuclei from Proposition~\ref{defn:nuclei-semilattice} by
considering only those nuclei that are Scott-continuous.
...
\end{prop}
\begin{proof}
  The construction is the same as the one in Proposition~\ref{defn:nuclei-semilattice}; the top
  ...
  \begin{align*}
    ...
    &\equiv j \paren{\bigvee_{i : I} x_i} \wedge k \paren{\bigvee_{j : I} x_j} & \\
    &= \paren{\bigvee_{i : I} j(x_i)} \wedge \paren{\bigvee_{j : I} k(x_j)} & [\text{Scott-continuity of $j$ and $k$}]\\
    &= \bigvee_{(i, j) : I \times I} j(x_i) \wedge k(x_j)                   & [\text{distributivity}]\\
    &= \bigvee_{i : I} j(x_i) \wedge k(x_i)                          & [\text{\dag}] \\
    &\equiv \bigvee_{i : I} (j \wedge k)(x_i).
  \end{align*}
  where, for the \dag\ step, we use antisymmetry. The backwards direction is immediate. For the forwards
  direction, we need to show that
  \begin{equation*}
    \bigvee_{(i, j) : I \times I} j(x_i) \wedge k(x_j) \quad\le\quad \bigvee_{i : I} j(x_i) \wedge k(x_i),
  \end{equation*}
  for which it suffices to show that $\bigvee_{i : I} j(x_i) \wedge k(x_i)$ is an upper bound of
  $\setof{j(x_i) \wedge k(x_j)}_{(i, j) : I \times I}$. Let $m, n : I$ be two arbitrary indices. As
  $\{x_i\}_{i : I}$ is directed, there must exist some $o$ such that $x_o$ is greater than both
  $x_m$ and $x_n$. Using the monotonicity of $j$ and $k$ (Proposition~\ref{prop:nuclei-mono}), we
  get
  \begin{equation*}
    j(x_m) \wedge k(x_n) \quad\le\quad j(x_o) \wedge k(x_o) \quad\le\quad \bigvee_{i : I} j(x_i) \wedge k(x_i)
  \end{equation*}
  as needed.
\end{proof}
